...
public class TopicResult extends JPanel {
    ...
    private void getRelatedPosts() {
        try {
            ...
            PreparedStatement stmt = con.prepareStatement("select author, datestamp, ptext, foto from post inner join topiccomment on post.author = topiccomment.cpauthor and post.datestamp = topiccomment.cpdate where tname = '" + topic.getName() + "' union select author, datestamp, ptext, foto from post inner join topicpost on post.author = topicpost.pauthor and post.datestamp = topicpost.pdate where tname = '" + topic.getName() + "' order by datestamp desc;");
            ...
            while (result.next()) {
                ...
                boolean belong = author.getUsername().equals(UserProfile.CURRENT_USER.getUsername());
                boolean show = true;
                if (!author.isPrivate()) {
                    stmt = con.prepareStatement(String.format("SELECT * FROM userrel where srcuser = '%s' and tgtuser = '%s' and status = 3", author.getUsername(), UserProfile.CURRENT_USER.getUsername()));
                    ResultSet set = stmt.executeQuery();
                    show = !set.next();
                    stmt.close();
                } else if(!belong){
                    stmt = con.prepareStatement(String.format("SELECT * FROM userrel where srcuser = '%s' and tgtuser = '%s' and status = 2", UserProfile.CURRENT_USER.getUsername(), author.getUsername()));
                    ResultSet set = stmt.executeQuery();
                    show = set.next();
                    stmt.close();
                }               
                if(show) {
                    ...
                    pcPanel.add(new PostScreen(post, home, belong));
                }
            }
            if(!found) {
                setVisible(false);
